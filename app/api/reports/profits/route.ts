...
export async function GET(req: NextRequest) {
  try {
    ...
    // Group by orders first to get unique orders
    ...
    // Calculate overall summary using actual order totals (not just item prices)
    // This ensures revenue matches what customers actually paid
    let totalRevenue = 0;
    let totalCost = 0;
    ...
    ordersWithProfits.forEach(order => {
      // Use the actual order total amount (includes fees, discounts, taxes)
      totalRevenue += parseFloat(order.totalAmount || '0');
      
      // Sum up costs and driver payments from items
      order.items.forEach((item: any) => {
        totalCost += item.profit.cost;
        totalDriverPayments += item.profit.driverPayment;
        totalItems++;
        
        if (item.profit.isProfit) {
          profitableItems++;
        } else {
          lossItems++;
        }
