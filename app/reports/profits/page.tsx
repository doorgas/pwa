...
export default function ProfitsReport() {
  ...
  // Temporary filter inputs (before apply button is clicked)
  const [tempStartDate, setTempStartDate] = useState('');
  const [tempEndDate, setTempEndDate] = useState('');
  const [tempSelectedProductId, setTempSelectedProductId] = useState('');
  ...
  useEffect(() => {
    // Only fetch when applied filters change, not temp inputs
    fetchProfitData();
  ...
  const setPresetDateRange = (preset: keyof typeof dateRanges) => {
    ...
    // Apply preset immediately (both temp and actual)
  ...
  const applyFilters = () => {
    ...
    setSelectedProductId(tempSelectedProductId);
  };
